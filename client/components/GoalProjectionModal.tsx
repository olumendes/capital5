...
export default function GoalProjectionModal({
  goal,
  children,
  open,
  onOpenChange,
}: GoalProjectionModalProps) {
  ...
  const [monthlyIncome, setMonthlyIncome] = useState(
    financialSummary.totalReceitas.toString(),
  );
  ...
  const getMonthlyExpenses = () => {
    if (expenseType === "budget") {
      ...
      const gastosFixosCategory = categoriesStatus.find(
        (cat) =>
      ...
      if (gastosFixosCategory) {
        ...
        return categoriesStatus.reduce(
          (total, cat) => total + cat.category.monthlyLimit,
          0,
        );
      }
    } else {
  ...
  const calculateProjection = () => {
    ...
    try {
      ...
      const monthsRemaining = Math.max(
        1,
        differenceInMonths(targetDate, currentDate) + 1,
      ); // +1 para incluir o mês atual
      ...
      const totalProjected =
        goal.currentAmount + fgtsToUse + monthlySavings * monthsRemaining;
      ...
      const isViable =
        totalProjected >= goal.targetAmount && monthlySavings >= 0;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto mx-4 w-[calc(100vw-2rem)] sm:w-full">
        <DialogHeader>
          ...
          <DialogDescription>
            Simule se conseguirá atingir seu objetivo na data planejada com sua
        ...
        <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Padrão: {formatCurrency(financialSummary.totalReceitas)}{" "}
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={expenseType}
                    onValueChange={(value) =>
                      setExpenseType(value as "budget" | "actual")
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                          ...
                          {(() => {
                            ...
                            const gastosFixosCategory = categoriesStatus.find(
                              (cat) =>
                                cat.category.name
                                  .toLowerCase()
                                  .includes("gastos fixos") ||
                                cat.category.name
                                  .toLowerCase()
                                  .includes("gastos fixo") ||
                                cat.category.name
                                  .toLowerCase()
                                  .includes("fixos"),
                            );
                            return formatCurrency(
                              gastosFixosCategory?.category.monthlyLimit || 0,
                            );
                          })()}
                        </span>
                      </SelectItem>
                    ...
                    (() => {
                      ...
                      if (!gastosFixosCategory) {
                        return (
                          <p className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded p-2">
                            ⚠️ Categoria "Gastos Fixos" não encontrada. Usando
                      ...
                      return (
                        <p className="text-xs text-green-600 bg-green-50 border border-green-200 rounded p-2">
                          ✅ Usando categoria "
                          {gastosFixosCategory.category.name}" -{" "}
                          {formatCurrency(
                            gastosFixosCategory.category.monthlyLimit,
                          )}
                        </p>
                      );
                    })()}
          ...
          {result && (
            ...
            >
              <CardHeader>
                <CardTitle
                  className={`text-lg flex items-center gap-2 ${result.isViable ? "text-green-800" : "text-red-800"}`}
                >
                  {result.isViable ? (
                    <>
                      <CheckCircle className="h-5 w-5" />✅ Objetivo Viável!
                    </>
                  ) : (
                    <>
                ...
                <div
                  className={`text-sm ${result.isViable ? "text-green-700" : "text-red-700"}`}
                >
                  {result.isViable ? (
                    <p>
                      ✨ Com sua sobra mensal de{" "}
                      {formatCurrency(result.monthlySavings)}, você conseguirá
                      atingir este objetivo
                      {result.monthsToReachGoal &&
                      result.monthsToReachGoal < result.monthsRemaining
              ...
              <CardContent className="space-y-4">
                {/* Resumo da Situação */}
                <div
                  className={`border rounded-lg p-4 ${result.isViable ? "border-green-300 bg-white" : "border-red-300 bg-white"}`}
                >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p
                        className={`font-semibold ${result.monthlySavings > 0 ? "text-green-600" : "text-red-600"}`}
                      >
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        {result.monthsRemaining} meses
                      </p>
                    </div>
                    <div>
                      <Label className="text-sm text-gray-600">
                        {result.isViable &&
                        result.totalProjected === goal.targetAmount
                          ? "Valor do Objetivo (Atingido)"
                          : "Valor Projetado"}
                      </Label>
                      <p
                        className={`font-semibold text-lg ${result.isViable ? "text-green-600" : "text-red-600"}`}
                      >
                    ...
                    <div>
                      ...
                      <p
                        className={`font-semibold ${result.isViable ? "text-green-600" : "text-red-600"}`}
                      >
                        {formatCurrency(
                          result.totalProjected - goal.targetAmount,
                        )}
                        ...
                        <Card className="border-blue-200 bg-blue-50">
                          ...
                          <CardContent>
                            ...
                            <p className="text-xs text-blue-600 mt-1">
                              ...
                              {formatCurrency(
                                result.monthlyIncomeNeeded -
                                  parseFloat(monthlyIncome.replace(",", ".")),
                              )}
                            </p>
                          </CardContent>
                        </Card>
                ...
                <div className="bg-gray-50 border rounded-lg p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span>
                        Gastos mensais (
                        {expenseType === "budget" ? "fixos" : "reais"}):
                      </span>
                      <span className="font-medium">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        ...
                        result.monthsToReachGoal < result.monthsRemaining
                          ? formatCurrency(goal.remainingAmount)
                          : formatCurrency(
                              result.monthlySavings * result.monthsRemaining,
                            )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between font-bold text-lg">
                      ...
                      <span
                        className={
                          result.isViable ? "text-green-600" : "text-red-600"
                        }
                      >
                        {formatCurrency(result.totalProjected)}
                      </span>
