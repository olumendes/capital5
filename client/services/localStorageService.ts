import { DBTransaction, DBCategory, DBInvestment, DBGoal, DBBudgetDivision } from '@shared/database-types';
...
const DEFAULT_CATEGORIES: DBCategory[] = [
  { id: 'cat-1', user_id: 'test-user-001', name: 'Salário', type: 'receita', is_default: true, created_at: new Date().toISOString(), icon: '💼', color: '#10b981' },
  { id: 'cat-2', user_id: 'test-user-001', name: 'Freelance', type: 'receita', is_default: true, created_at: new Date().toISOString(), icon: '💻', color: '#3b82f6' },
  { id: 'cat-3', user_id: 'test-user-001', name: 'Investimentos', type: 'receita', is_default: true, created_at: new Date().toISOString(), icon: '📈', color: '#8b5cf6' },
  { id: 'cat-4', user_id: 'test-user-001', name: 'Alimentação', type: 'despesa', is_default: true, created_at: new Date().toISOString(), icon: '🍔', color: '#f59e0b' },
  { id: 'cat-5', user_id: 'test-user-001', name: 'Transporte', type: 'despesa', is_default: true, created_at: new Date().toISOString(), icon: '🚗', color: '#ef4444' },
  { id: 'cat-6', user_id: 'test-user-001', name: 'Moradia', type: 'despesa', is_default: true, created_at: new Date().toISOString(), icon: '🏠', color: '#6366f1' },
  { id: 'cat-7', user_id: 'test-user-001', name: 'Saúde', type: 'despesa', is_default: true, created_at: new Date().toISOString(), icon: '⚕️', color: '#06b6d4' },
  { id: 'cat-8', user_id: 'test-user-001', name: 'Educação', type: 'despesa', is_default: true, created_at: new Date().toISOString(), icon: '📚', color: '#14b8a6' },
];
...
class LocalStorageService {
  ...
  async createTransaction(transaction: Omit<DBTransaction, 'id' | 'created_at' | 'updated_at'>): Promise<DBTransaction> {
    if (!this.isTestMode()) throw new Error('Not in test mode');
  ...
  async updateTransaction(id: string, updates: Partial<DBTransaction>): Promise<DBTransaction> {
    if (!this.isTestMode()) throw new Error('Not in test mode');
  ...
  async createCategory(category: Omit<DBCategory, 'id' | 'created_at'>): Promise<DBCategory> {
    if (!this.isTestMode()) throw new Error('Not in test mode');
  ...
  async createInvestment(investment: Omit<DBInvestment, 'id' | 'created_at' | 'updated_at'>): Promise<DBInvestment> {
    if (!this.isTestMode()) throw new Error('Not in test mode');
  ...
  async updateInvestment(id: string, updates: Partial<DBInvestment>): Promise<DBInvestment> {
    if (!this.isTestMode()) throw new Error('Not in test mode');
  ...
  async createBudgetDivision(division: Omit<DBBudgetDivision, 'id' | 'created_at'>): Promise<DBBudgetDivision> {
    if (!this.isTestMode()) throw new Error('Not in test mode');
  ...
  async updateBudgetDivision(id: string, updates: Partial<DBBudgetDivision>): Promise<DBBudgetDivision> {
    if (!this.isTestMode()) throw new Error('Not in test mode');
