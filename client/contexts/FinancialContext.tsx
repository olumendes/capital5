...
function financialReducer(
  ...
  switch (action.type) {
    ...
    case "SET_TRANSACTIONS":
      ...
      return {
        ...newState,
        summary: calculateSummary(action.payload, state.categories),
      };
...
export function FinancialProvider({ children }: FinancialProviderProps) {
  ...
  const updateFGTSBalance = async (amount: number) => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Erro ao atualizar saldo FGTS";
  ...
  const addTransaction = async (
    ...
    try {
      ...
      const transaction: Transaction = {
        ...
        sourceDetails: dbTransaction.source_details
          ? JSON.parse(dbTransaction.source_details)
          : undefined,
    ...
    try {
      ...
      const transactions: Transaction[] = dbTransactions.map(
        (dbTx: DBTransaction) => ({
          ...
          categoryName: dbCategories.find(
            (cat: DBCategory) => cat.id === dbTx.category_id,
          )?.name,
          description: dbTx.description,
          amount: dbTx.amount,
          date: dbTx.date,
          source: dbTx.source,
          sourceDetails: dbTx.source_details
            ? JSON.parse(dbTx.source_details)
            : undefined,
